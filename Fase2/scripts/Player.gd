...
func _physics_process(delta: float) -> void:
	...
	if direction != 0:
		...
		$Estilingue.scale.x    = 1 if facing_right else -1
		$AttackArea.position.x = 30 if facing_right else -30
		$Espetao.position.x    = 18 if facing_right else -18
		$Estilingue.position.x = 18 if facing_right else -18
		if not is_attacking:
			$Espetao.rotation_degrees    = -40.0 if facing_right else 40.0
			$Estilingue.rotation_degrees = -42.0 if facing_right else 42.0
	else:
...
func _update_weapon_visual() -> void:
	$Espetao.visible   = (current_weapon == "espetao")
	$Estilingue.visible = (current_weapon == "estilingue")
	# Reset rotations
	var sign_dir = 1.0 if facing_right else -1.0
	$Espetao.rotation_degrees    = -40.0 * sign_dir
	$Estilingue.rotation_degrees = -42.0 * sign_dir
